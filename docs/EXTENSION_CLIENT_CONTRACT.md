...
- **Behaviour change — SSE keep-alive cadence 40 s → 15 s.** `GET /api/extension/live` now sends an SSE comment ping every 15 s by default (configurable server-side via `REALTIME_LIVE_HEARTBEAT_MS`). This keeps the stream alive under proxies that idle-close at 30 s. **Client action:** none — comment lines are ignored by `EventSource`.
